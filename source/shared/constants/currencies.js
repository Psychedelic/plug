...
const cleanTokenStrings = (token) => {
  return {
    ...token,
    name: removeNullBytes(token?.name),
    symbol: removeNullBytes(token?.symbol),
  };
};
...
export const formatAssets = (assets = [], icpPrice) => {
  const mappedAssets = assets.map(({ amount, token, error }) => {
    const cleanStringToken = cleanTokenStrings(token);
    const asset = formatAssetBySymbol(
      amount,
      cleanStringToken.symbol,
      icpPrice
    );
    return {
      ...cleanStringToken,
