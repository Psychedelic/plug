...
export const parseMnemonic = (mnemonic) =>
  (mnemonic || "").trim().toLowerCase().match(/\w+/gu)?.join(" ") || "";
...
const ImportWalletStep = ({ handleNextStep, handleSetMnemonic }) => {
  ...
  const [showMnemonic, setShowMnemonic] = useState(
    createArray(MNEMONIC_LENGTH, false)
  );
  ...
  const onMnemonicChange = useCallback(
    (newDraftMnemonic) => {
      ...
      const joinedDraftMnemonic = newDraftMnemonic
        .join(" ")
        .trim()
        .toLowerCase();
        ...
        {[...Array(MNEMONIC_LENGTH).keys()].map((index) => {
          ...
          return (
            <div key={index} className={classes.mnemonicWordInputContainer}>
              <Typography className={classes.mnemonicWordLabel}>
                {`${index + 1}.`}
              </Typography>
              <div className={classes.mnemonicWordAction}>
                <TextInput
                  id={id}
